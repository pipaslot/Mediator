...
public class JsonContractSerializer_CredibilityTests : Pipaslot.Mediator.Http.Tests.Serialization.ContractSerializer_CredibilityTestBase
{
    ...
    public void Request_InterfaceCollection_ShouldCallVerifyCredibility()
    {
        ...
        CredibleProviderMock.Setup(p => p.VerifyCredibility(action.GetType()));
        CredibleProviderMock.Setup(p => p.VerifyCredibility(contract.GetType()));
