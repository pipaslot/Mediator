...
- **Design principle: nested calls must not have side effects on the shared `HttpContext`.** A nested `MediatorContext` (`IsNested == true`) is not the owner of the request's `HttpContext` — the same action/handler is meant to be callable directly as a root HTTP action *or* reused as a nested call purely for its result, and reuse must not leak into the response the root caller is building (response body, status code, headers, ...). Only the root context, applied centrally from `Pipaslot.Mediator.Http.MediatorMiddleware` after the whole pipeline finishes, may touch `HttpContext`. A nested failure should surface through the normal mediator API instead — `context.Status = Failed` plus a `Notification` (see `Notification.StopPropagation` to keep it from also failing the parent context), or a thrown exception for `*Unhandled` calls. Apply this whenever designing or reviewing a middleware/handler/feature that would otherwise reach for `HttpContext`/`IHttpContextAccessor` directly (see `Todos/2.1-analysis-nested-call-http-direct-response-write.md` and `Todos/2.2-analysis-response-status-code-hint.md` for two concrete cases this ruled out).
