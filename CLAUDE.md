...
The `<summary>`/`<remarks>`/`<example>` blocks on the core public types are a **deliberate delivery channel, not decoration**: `GenerateDocumentationFile` ships `Pipaslot.Mediator.xml` inside the NuGet package, so this is the only documentation an AI coding agent (or IntelliSense user) in a consumer project has without fetching anything. Every fact placed here reaches a team that has never opened the wiki.

- **The overlap with `docs/wiki/` is intentional — do not "deduplicate" it.** The two channels have different audiences and different failure modes: the wiki explains a topic end-to-end for someone reading it deliberately; the XML doc answers "what is this and am I using it right?" for someone who has only the type in front of them. A future pass that trims a `<remarks>` because "the wiki already says this" removes the copy that actually travels with the package.
- **`<remarks>` carries "when to use it / what to use instead", not a restatement of the signature.** Reserve `<see cref=""/>` for the alternative a reader should consider, so every type with a "use X instead" path names it. Where the library's naming collides with MediatR (`IRequest`, `IMessage`, `IRequestHandler`, `IMessageHandler`, `IMediator`'s methods), say explicitly how the contract differs — a wrong assumption there is the single most likely mistake in a consumer project.
- **Never replace these with `<inheritdoc/>`.** The C# compiler copies the tag into the shipped `.xml` verbatim instead of expanding it; resolution happens in the consumer's IDE. A member documented only by `<inheritdoc/>` ships with zero words for an agent reading the package. It is fine for a class restating a member it implements, and for repeated `<param>` text via `path="/summary"`.
- **Priority set** (the types worth this investment, in the order a reader trips over them): `IMediatorAction`/`IMediatorHandler` and their `IRequest`/`IMessage`/`IRequestHandler`/`IMessageHandler` naming layer, `IMediator`'s four entry points, `IMediatorExceptionHandler`/`IMediatorExceptionContext`, `IMediatorMiddleware`/`IExecutionMiddleware`/`MediatorContext`, `ICredibleProvider` and its options, `Use`/`UseWhen`/`AddPipeline`, then authorization (`IHandlerAuthorization`, `Policy`, `RuleSet`) and the exception types. Contract detail belongs on the root (`IMediatorAction`/`IMediatorHandler`); the naming layer above it says only that it *is* a naming layer, since the mediator dispatches on the roots and consumers may define their own `ICommand`/`IQuery`.
