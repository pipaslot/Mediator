...
* Added `MediatorContext.Create`, building a context outside the mediator so a custom middleware or exception handler can be unit-tested without a service container — see [6.-Pipelines-and-Middlewares.md](6.-Pipelines-and-Middlewares.md#how-to-unit-test-a-custom-middleware).
...
* Added the opt-in `LegacyExceptionMessageHandler`, restoring the pre-safe-by-default behavior of reporting an exception's own `Message` to the client — see [6.2.-Exception-handling.md](6.2.-Exception-handling.md#restore-the-pre-safe-by-default-message-behavior-temporarily).
...
* `MediatorExecutionException.CreateForUnhandledError` (both overloads) was removed; use `MediatorUnhandledErrorException.Create` instead.
* `ReduceDuplicateProcessingMiddleware` and `MiddlewareRegistratorExtensions.UseReduceDuplicateProcessing` were removed; implement your own middleware with an explicit dedup key instead.
* `Pipaslot.Mediator.Abstractions.RequestGenericHelpers` was removed.
* `ServiceProviderExtensions` is now `internal` and can no longer be used from outside the library.
