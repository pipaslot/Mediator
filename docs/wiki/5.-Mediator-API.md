...
## Diagnostics
The [`Pipaslot.Mediator.Analyzers`](https://github.com/pipaslot/Mediator) package (bundled with `Pipaslot.Mediator`, no separate install) flags the following mistakes at build time.

### Discarding the response (PIPMED002)
Neither `Dispatch` nor `Execute` throws on failure - the only way to learn about one is to read the returned [Response](2.-Core-concepts-and-glossary.md#response)'s `Success`/`Failure`. A statement-level call whose result is never assigned anywhere, `await mediator.Dispatch(x);` or `mediator.Dispatch(x);` as a bare statement, silently drops that failure - there is nothing else that would surface it. The Roslyn analyzer shipped with the package (`PIPMED002`) flags this and offers three code fixes: capture the result into a local and guard the rest of the method with `if (result.Failure) { ... }`, switch the call to [`DispatchUnhandled`/`ExecuteUnhandled`](5.-Mediator-API.md#executeunhandled-and-dispatchunhandled) above (which throw on failure instead of reporting it through a response nothing reads), or rewrite the call to the idiomatic explicit discard `_ = await mediator.Dispatch(x);` for a caller that has already decided the response genuinely doesn't matter. A call whose result is assigned to a variable, even if that variable's `Success`/`Failure` is never read afterwards, is out of scope for `PIPMED002` in this version - only the bare-statement shape is detected. The explicit discard `_ = await mediator.Dispatch(x);` is never flagged in the first place.
