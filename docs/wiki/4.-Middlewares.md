...
You can pass custom parameters to middeleware to change the behaviour depending on the pipeline usage.
``` 
services.AddMediator()
    .AddPipeline<...>(p => p
...
public class MyMiddleware : IMediatorMiddleware
{
    ...
    {
        var feature = context.Features.Get<MiddlewarePrametersFeature>();
