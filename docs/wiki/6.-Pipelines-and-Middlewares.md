...
`AddHandlers`/`AddHandlersFromAssemblyOf<T>` take `serviceLifetime` as `ServiceLifetime?`. Omitting it (or passing `null`) lets `ISingleton`/`IScoped` dictate the lifetime freely. Passing any explicit `ServiceLifetime` - including `ServiceLifetime.Transient` - for a handler implementing `ISingleton`/`IScoped` is treated as a conflicting registration and throws, the same as passing `Scoped` or `Singleton` explicitly.
