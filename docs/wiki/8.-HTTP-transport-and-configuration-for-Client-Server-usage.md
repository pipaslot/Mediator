...
Replacing an exception with a safe client message is a job for a typed exception handler rather than a middleware, because a middleware that catches an exception also hides it from that logging:
```csharp
public class StorageExceptionHandler : IMediatorExceptionHandler<StorageException>
{
    public Task<string> Handle(StorageException exception, CancellationToken cancellationToken)
        => Task.FromResult("Operation failed, please contact administrator");
}
```
```csharp
services.AddMediatorServer()
