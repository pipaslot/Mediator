...
`ServerMediatorOptions.AllowHttpGetWhen(Func<IMediatorAction, bool> condition)` registers a condition deciding whether a given action instance may be invoked over HTTP GET; when multiple conditions are registered, an action is allowed if at least one of them returns true. **No condition registered by default** keeps this backward compatible - every action registered with the mediator can still be invoked over GET, exactly as before. Register a condition for only the actions that are actually meant to be triggered from a hyperlink or embedded resource (typically read-only queries powering a file download):
```
services.AddMediatorServer(o => o
        .AllowHttpGetWhenAction<DownloadFileQuery>())
    .AddActionsFromAssemblyOf<MyAction>();
```
`AllowHttpGetWhenAction<TAction>()` is a shortcut for `AllowHttpGetWhen(a => typeof(TAction).IsAssignableFrom(a.GetType()))`. Registering any condition turns on `ServerMediatorOptions.RestrictHttpGetToAllowedActions` as a side effect - it is a computed, read-only property (`true` once at least one condition has been registered), not a flag you set directly. Either way, only GET is affected - POST is never restricted by this mechanism.
