...
`ServerMediatorOptions.HttpGetConditions` is a `HttpGetActionAllowlist` holding the conditions that decide whether a given action instance may be invoked over HTTP GET. `HttpGetConditions.Allow(Func<IMediatorAction, bool> condition)` registers one; when multiple conditions are registered, an action is allowed if at least one of them returns true. **Empty by default** keeps this backward compatible - every action registered with the mediator can still be invoked over GET, exactly as before. A common pattern is a marker interface for the actions that are safe to trigger from a hyperlink or embedded resource (typically read-only queries powering a file download), as in the [Demo app](https://github.com/pipaslot/Mediator/tree/main/Demo/Shared/Playground/IHttpGetAllowed.cs):
```
...
`Allow` is additive - each call registers one more condition, it never removes an earlier one. When configuration is composed across multiple steps (e.g. a shared library exposes an `Action<ServerMediatorOptions>` that registers a condition, and the application composing it afterwards wants to correct or narrow it), call `HttpGetConditions.Clear()` first to remove everything registered so far, then `Allow` again for the conditions that should replace them. Clearing with nothing re-added restores the unrestricted default:
```
void ConfigureSharedDefaults(ServerMediatorOptions o) =>
    o.HttpGetConditions.Allow(action => action is IHttpGetAllowed); // registered by a shared setup step

services.AddMediatorServer(o =>
    {
        ConfigureSharedDefaults(o);
        // a later configuration step overrides the shared step's conditions entirely
        o.HttpGetConditions.Clear();
        o.HttpGetConditions.Allow(action => action is IHttpGetAllowed && action is not LegacyExportQuery);
    })
    .AddActionsFromAssemblyOf<MyAction>();
```
`HttpGetConditions.Conditions` exposes the currently registered conditions for inspection. There is no separate flag for "is GET restricted at all" - check `options.HttpGetConditions.Conditions.Any()` when you need that.
