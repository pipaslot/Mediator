...
`ServerMediatorOptions.RestrictHttpGetToAllowedActionTypes` restricts which action types may be invoked over HTTP GET to an explicit allowlist. **Disabled by default** to stay backward compatible - every action type registered with the mediator can still be invoked over GET, exactly as before. Enable it and allowlist only the action types that are actually meant to be triggered from a hyperlink or embedded resource (typically read-only queries powering a file download):
```
services.AddMediatorServer(o => o
        .AddAllowedHttpGetActionType<DownloadFileQuery>())
    .AddActionsFromAssemblyOf<MyAction>();
```
`AddAllowedHttpGetActionAssemblyOf<T>()`/`AddAllowedHttpGetActionAssembly(...)` allowlist every action type in an assembly instead of one type at a time. Each of these, like `RestrictHttpGetToAllowedActionTypes = true` directly, only affects GET - POST is never restricted by this option.

A GET request for an action type outside the allowlist fails with a `MediatorHttpException` surfaced through the normal error response, the same way an unrecognized/non-credible type does.

> This option is planned to become enabled by default in the next major version. Until then, review which of your actions are reachable over GET and allowlist explicitly - do not rely on the current permissive default for anything that changes state.
