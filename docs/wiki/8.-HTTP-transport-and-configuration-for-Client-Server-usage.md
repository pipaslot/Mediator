...
### HTTP GET action allowlist (CSRF protection)

`MediatorMiddleware` accepts action calls over HTTP GET as well as POST (see [Custom HTTP responses and file download](9.3.-Custom-HTTP-responses-and-file-download.md)), so that a download link can be a plain `<a>`/`<img>` tag. A GET request, unlike POST, can be triggered cross-site without any consent from the caller - a page you don't control can embed `<img src="https://yourapp/_mediator/request?action=...">` and the victim's browser will send it with their cookies/Windows authentication attached. This makes any state-changing action reachable over GET a CSRF risk, most dangerously for an authenticated administrator.

`ServerMediatorOptions.AllowHttpGetWhen(Func<IMediatorAction, bool> condition)` registers a condition deciding whether a given action instance may be invoked over HTTP GET; when multiple conditions are registered, an action is allowed if at least one of them returns true. **No condition registered by default** keeps this backward compatible - every action registered with the mediator can still be invoked over GET, exactly as before. A common pattern is a marker interface for the actions that are safe to trigger from a hyperlink or embedded resource (typically read-only queries powering a file download), as in the [Demo app](https://github.com/pipaslot/Mediator/tree/main/Demo/Shared/Playground/IHttpGetAllowed.cs):
```
public interface IHttpGetAllowed;

public record DownloadFileQuery(int FileId) : IRequest<FileResult>, IHttpGetAllowed;

services.AddMediatorServer(o => o
        .AllowHttpGetWhen(action => action is IHttpGetAllowed))
    .AddActionsFromAssemblyOf<MyAction>();
```
Only GET is affected - POST is never restricted by this mechanism.

A GET request for an action no registered condition allows fails with a `MediatorHttpException` surfaced through the normal error response, the same way an unrecognized/non-credible type does.

> With no condition ever registered, every action is allowed over GET (today's permissive default). The next major version is planned to instead deny every action over GET while no condition is registered. Until then, review which of your actions are reachable over GET and register a condition explicitly for the ones that are safe - do not rely on the current permissive default for anything that changes state.

#### Replacing or clearing conditions

`AllowHttpGetWhen` is additive - each call registers one more condition, it never removes an earlier one. When configuration is composed across multiple steps (e.g. a shared library exposes an `Action<ServerMediatorOptions>` that registers a condition, and the application composing it afterwards wants to correct or narrow it), read or replace the whole set through `ServerMediatorOptions.HttpGetConditions` instead. Its getter returns the currently registered conditions; its setter clears and replaces them wholesale, mirroring how [`CredibleResultTypes`](8.-HTTP-transport-and-configuration-for-Client-Server-usage.md#typenamehandling-and-security) works for the credible-type allowlist. Assigning an empty array restores the unrestricted default:
```
void ConfigureSharedDefaults(ServerMediatorOptions o) =>
    o.AllowHttpGetWhen(action => action is IHttpGetAllowed); // registered by a shared setup step

services.AddMediatorServer(o =>
    {
        ConfigureSharedDefaults(o);
        // a later configuration step overrides the shared step's condition entirely
        o.HttpGetConditions = [action => action is IHttpGetAllowed && action is not LegacyExportQuery];
    })
    .AddActionsFromAssemblyOf<MyAction>();
```
There is no separate flag for "is GET restricted at all" - check `options.HttpGetConditions.Any()` when you need that.
